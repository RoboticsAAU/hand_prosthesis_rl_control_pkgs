#include "depth_camera_gazebo_plugin/gazebo_ros_depth_camera.h"
...
namespace gazebo {
// Register the plugin
GZ_REGISTER_MODEL_PLUGIN(GazeboRosDepthCamera)

GazeboRosDepthCamera::GazeboRosDepthCamera() {}

GazeboRosDepthCamera::~GazeboRosDepthCamera() {
  ROS_DEBUG_STREAM_NAMED("DepthCamera_camera", "Unloaded");
}

void GazeboRosDepthCamera::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  ROS_INFO("DepthCamera Gazebo ROS plugin loading.");
...
void GazeboRosDepthCamera::OnNewFrame(const rendering::CameraPtr cam, const transport::PublisherPtr pub) 
{
...
bool GazeboRosDepthCamera::FillPointCloudHelper(sensor_msgs::PointCloud2 &point_cloud_msg, uint32_t rows_arg, uint32_t cols_arg,  uint32_t step_arg, void *data_arg)
{
...
void GazeboRosDepthCamera::OnNewDepthFrame() {
  ...
  DepthCameraPlugin::OnNewDepthFrame();
